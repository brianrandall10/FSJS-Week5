...
$.getJSON( "http://pokeapi.co/api/v2/pokemon/1/?offset=20", function( pokeData ) {
  });
...
function addPokemon(name) {
    $(
	'<ul>';
	$.each(name.results), function (i, pokemon) {
        list += '<li class="poke-card">';
        list += '<h3 class="name">' + pokemon.name + '</h3>';
        list += '</li>';
	}
	'</ul>';
    ).appendTo('#pokemon'));
